<?php
class ModelExtensionShippingNdPasts extends Model {
	...
	public function getQuote($address) {
		...
		$cost_value = str_replace(',', '.', trim((string)$this->config->get('shipping_nd_pasts_cost')));
		if ($cost_value === '' || !is_numeric($cost_value) || (float)$cost_value < 0) {
			return array();
		}

		$cost = (float)$cost_value;
		$free_total = str_replace(',', '.', trim((string)$this->config->get('shipping_nd_pasts_free_total')));
		if ($free_total !== '' && is_numeric($free_total) && (float)$free_total > 0 && $this->cart->getSubTotal() >= (float)$free_total) {
			$cost = 0.0;
		}

		$tax_class_id = (int)$this->config->get('shipping_nd_pasts_tax_class_id');
		$selected_id = isset($this->session->data['nd_pasts_terminal_id']) ? (string)$this->session->data['nd_pasts_terminal_id'] : '';

		$quote_data = array(
			'terminal' => array(
				'code' => 'nd_pasts.terminal',
		...
		return array(
			...
			'quote' => $quote_data,
			'sort_order' => (int)$this->config->get('shipping_nd_pasts_sort_order'),
			'error' => false
		);
	}
	...
	public function getTerminals($country) {
		...
		$cache_file = rtrim(DIR_CACHE, '/\\') . DIRECTORY_SEPARATOR . 'nd_pasts_' . strtolower($country) . '.json';
		$cached = $this->readCache($cache_file);
		$ttl = (int)$this->config->get('shipping_nd_pasts_cache_ttl');
	...
	private function isAvailableInGeoZone($address) {
		$geo_zone_id = (int)$this->config->get('shipping_nd_pasts_geo_zone_id');
		if (!$geo_zone_id) {
			return true;
		}
	...
	private function fetchJson($url) {
		...
		curl_setopt($curl, CURLOPT_USERAGENT, 'ND-Pasts-OpenCart/1.0');
	...
	private function renderSelector($terminals, $selected_id) {
		$search_id = 'nd-pasts-search';
		$select_id = 'nd-pasts-terminal';
		$html = '<div class="nd-pasts-selector">';
		$html .= '<span class="nd-pasts-method">' . $this->escape($this->language->get('text_method')) . '</span>';
		...
		$html .= '<select name="nd_pasts_terminal" id="' . $select_id . '" class="form-control">';
		...
		$html .= '</select></div>';
		$html .= '<script>(function(){var q=document.getElementById("' . $search_id . '");var s=document.getElementById("' . $select_id . '");if(!q||!s||s.getAttribute("data-ready")){return;}s.setAttribute("data-ready","1");var o=Array.prototype.slice.call(s.options,1);q.addEventListener("input",function(){var v=this.value.toLocaleLowerCase();o.forEach(function(x){x.hidden=v!==""&&x.text.toLocaleLowerCase().indexOf(v)===-1;});});s.addEventListener("change",function(){var r=document.querySelector("input[name=shipping_method][value=\\"nd_pasts.terminal\\"]");if(r){r.checked=true;}});})();</script>';
